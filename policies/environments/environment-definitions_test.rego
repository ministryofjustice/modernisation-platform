...
test_unexpected_access if {
  deny["`example.json` uses an unexpected access level: got `incorrect-access`, expected one of: administrator, analytics-engineer, data-engineer, data-scientist, developer, instance-access, instance-management, migration, mwaa-user, platform-engineer-adminread-only, reporting-operations, sandbox, security-audit, view-only, powerbi-user, fleet-manager, quicksight-admin"] with input as { "filename": "example.json", "environments": [{"access": [{"level": "incorrect-access"}]}]}
}
