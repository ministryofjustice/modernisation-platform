  ...
  do
    ...
    do
      ...
      then
        ...
        teams=$(jq -r --arg e "${env}" '.environments[] | select( .name == $e ) | .access[] | select(.github_action_reviewer=="true") | (.github_slug // .sso_group_name)' $json_file)
        # if teams is empty (none of the teams have the github_action_reviewer flag), get all teams
        if [ -z "$teams" ]; then
          teams=$(jq -r --arg e "${env}" '.environments[] | select( .name == $e ) | .access[] | (.github_slug // .sso_group_name)' $json_file)
        fi
