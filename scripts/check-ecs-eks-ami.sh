...
echo "Account ID, Account Name,Instance ID,AMI ID,Service Type (ECS/EKS),Timestamp" > $OUTDATED_AMIS_FILE
        ...
        for ecs_cluster in $ecs_clusters; do
            ...
            if [[ -z "$ecs_instances" ]]; then
                ...
                for ecs_instance_arn in $ecs_instances; do
                    ...
                    ecs_ami_id=$(aws ec2 describe-instances --instance-ids $ecs_instance_id --query 'Reservations[*].Instances[*].ImageId' --output text)
                    if [[ "$ecs_ami_id" != "$latest_ecs_ami" ]]; then
        ...
        for eks_cluster in $eks_clusters; do
            ...
            if [[ -z "$eks_node_groups" ]]; then
                ...
                for eks_node_group in $eks_node_groups; do
                    ...
                    eks_instance_ids=$(aws ec2 describe-instances --filters "Name=tag:eks:nodegroup-name,Values=$eks_node_group" --query 'Reservations[*].Instances[*].InstanceId' --output text)
                    if [[ -z "$eks_instance_ids" ]]; then
                        echo "No EKS instances found in node group $eks_node_group."
                    else
