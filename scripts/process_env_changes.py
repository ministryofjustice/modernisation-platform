...
def apply_environment_change(root: Dict[str, Any], change_type: str, name: str, payload: Any):
    ...
    if change_type in ("addition", "update"):
        if not name:
            print("ERROR: environment-name required", file=sys.stderr)
            sys.exit(1)
        if payload is None or not isinstance(payload, dict):
            print("ERROR: environment-payload must be object", file=sys.stderr)
            sys.exit(1)
...
def apply_tags_change(root: Dict[str, Any], change_type: str, payload: Any, remove_keys: List[str]):
    ...
    if change_type in ("addition", "update"):
        if payload is None or not isinstance(payload, dict):
            print("ERROR: tags-payload must be object", file=sys.stderr)
            sys.exit(1)
...
def apply_other_change(root: Dict[str, Any], change_type: str, payload: Any, remove_keys: List[str]):
    ...
    if change_type == "removal":
        for k in remove_keys:
            if k in reserved:
                print(f"ERROR: cannot remove reserved key '{k}'", file=sys.stderr)
                sys.exit(1)
            root.pop(k, None)
    ...
    if change_type in ("addition", "update"):
        if payload is None or not isinstance(payload, dict):
            print("ERROR: other-payload must be object", file=sys.stderr)
            sys.exit(1)
        for k in payload:
            if k in reserved:
                print(f"ERROR: other-payload cannot include reserved key '{k}'", file=sys.stderr)
                sys.exit(1)
...
def main():
    ...
    p.add_argument("--environments-change-type", required=True, choices=["skip","addition","update","removal"])
    p.add_argument("--environment-name", default="")
    p.add_argument("--environment-payload", default="")
    p.add_argument("--tags-change-type", required=True, choices=["skip","addition","update","removal"])
    p.add_argument("--tags-payload", default="")
    p.add_argument("--tags-remove-keys", default="")
    p.add_argument("--other-change-type", required=True, choices=["skip","addition","update","removal"])
