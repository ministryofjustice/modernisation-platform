...
if [ -f final_users.list ]; then
    while read username; do
        ...
        gh issue create --title "Inactive IAM Collaborator Deletion Required - Username: $username" --label security --project "Modernisation Platform" --body "The IAM-Collaborator-Inactivity-Monitoring workflow has detected that the username $username is inactive for more than 180 days. 
