...
if [ -f final_users.list ]; then
    while read username; do
        ...
        if [ -z "$open_issue" ]; then
          ...
          gh issue create --title "Inactive IAM Collaborator Deletion Required - Username: $username" --label security --project "Modernisation Platform" --body "The [IAM-Collaborator-Inactivity-Monitoring workflow](${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }}) has detected that the username $username is inactive for more than 180 days. 
