...
The [status-page-subscriptions.tf](status-page-subscriptions.tf) automatically manages email subscriptions to the Modernisation Platform external status page.

### How it works

- Automatically subscribes all `infrastructure-support` email addresses from `environments/*.json` files to the status page
- The list of subscribers can be seen [here](https://moj-digital-tools.pagerduty.com/external-status-dashboard/PN2KMAZ/subscribers)

### Adding additional emails

To add ad-hoc emails that aren't in the `environments/*.json` files, add them to the `additional_subscriber_emails` list in [status-page-subscriptions.tf](status-page-subscriptions.tf):

```hcl
additional_subscriber_emails = [
  "team-email@digital.justice.gov.uk",
  "external-partner@example.com",
]
```

The script will automatically subscribe these emails on the next `terraform apply`.
