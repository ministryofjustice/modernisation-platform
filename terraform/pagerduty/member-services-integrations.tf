...
# Delius Alfresco - Non Prod

resource "pagerduty_service" "alfresco_nonprod" {
  name                    = "Delius Alfresco Non Prod"
  description             = "Delius Alfresco Non Prod Alarms"
  auto_resolve_timeout    = 345600
  acknowledgement_timeout = "null"
  escalation_policy       = pagerduty_escalation_policy.member_policy.id
  alert_creation          = "create_alerts_and_incidents"
...
resource "pagerduty_service_integration" "alfresco_nonprod_cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  service = pagerduty_service.alfresco_nonprod.id
  vendor  = data.pagerduty_vendor.cloudwatch.id
}

resource "pagerduty_slack_connection" "alfresco_nonprod_connection" {
  source_id         = pagerduty_service.alfresco_nonprod.id
  source_type       = "service_reference"
  workspace_id      = local.slack_workspace_id
  channel_id        = "CRT7QGKNZ"
  notification_type = "responder"
  lifecycle {
    ignore_changes = [
      config,
