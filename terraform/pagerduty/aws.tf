...
resource "aws_secretsmanager_secret_version" "pagerduty_integration_keys" {
  ...
  secret_string = jsonencode({
    ...
    laa_mlra_alarms        = pagerduty_service_integration.laa_mlra_cloudwatch.integration_key,
    laa_oas_alarms         = pagerduty_service_integration.laa_oas_cloudwatch.integration_key
  })
}
