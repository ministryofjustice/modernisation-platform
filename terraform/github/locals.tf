...
locals {
  ...
  # Extract GitHub or SSO slugs
  github_or_sso_slugs = [
    for access in local.filtered_access : 
      coalesce(lookup(access, "github_slug", null), lookup(access, "sso_group_name", null))
  ]

  # Final list including all-org-members and distinct slugs
