  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = aws_lambda_function.prefix_transform.arn
      }
    ]
  })
}
    ...
    processing_configuration {
      ...
      processors {
        ...
        parameters {
          ...
          parameter_value = "${aws_lambda_function.prefix_transform.qualified_arn}"
        }
      }
    }
  }
...
resource "aws_iam_role" "firehose_lambda_role" {
  name = "firehose_lambda_prefix_role"
...
resource "aws_lambda_function" "prefix_transform" {
  filename         = "${path.module}/lambda/lambda_firehose_prefix_transform.zip"
  function_name    = "firehose-prefix-transform"
  role             = aws_iam_role.firehose_lambda_role.arn
  handler          = "lambda_firehose_prefix_transform.lambda_handler"
  runtime          = "python3.11"
  timeout          = 60
  source_code_hash = filebase64sha256("${path.module}/lambda/lambda_firehose_prefix_transform.zip")
  publish          = true
}
