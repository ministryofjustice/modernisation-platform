...
data "aws_iam_policy_document" "oidc_assume_plan_role_member" {
  ...
  statement {
    ...
    effect    = "Allow"
    resources = [
      "arn:aws:glue:*:${local.environment_management.account_ids[terraform.workspace]}:catalog",
      "arn:aws:glue:*:${local.environment_management.account_ids[terraform.workspace]}:connection/*"
    ]
    actions = [
      "glue:GetConnection"
    ]
