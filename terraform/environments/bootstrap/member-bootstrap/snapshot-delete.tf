...
module "instance-scheduler-access" {
  ...
  additional_trust_roles = [format("arn:aws:iam::%s:role/SnapshotDeleteLambdaFunctionPolicy", local.environment_management.account_ids["core-shared-services-production"])]
  policy_arn             = aws_iam_policy.instance-scheduler-access[0].id
  role_name              = "SnapshotDeleteAccess"
}

#tfsec:ignore:aws-iam-no-policy-wildcards
data "aws_iam_policy_document" "snapshot-delete-access" {
  statement {
...
resource "aws_iam_policy" "snapshot-delete-access" {
...
module "testing_snapshot-delete-access" {
  ...
  source                 = "github.com/ministryofjustice/modernisation-platform-terraform-cross-account-access?ref=6819b090bce6d3068d55c7c7b9b3fd18c9dca648" #v3.0.0
  account_id             = local.environment_management.account_ids["core-shared-services-production"]
  additional_trust_roles = [format("arn:aws:iam::%s:role/SnapshotDeleteLambdaFunctionPolicy ", local.environment_management.account_ids["core-shared-services-production"]), format("arn:aws:iam::%s:root", local.environment_management.account_ids["testing-test"])]
  policy_arn             = aws_iam_policy.instance-scheduler-access[0].id
  role_name              = "SnapshotDeleteAccess"
}
