...
provider "aws" {
  region = "eu-west-2"
  assume_role {
    role_arn = contains(data.aws_caller_identity.original_session.arn, "githubactionssession") ? "arn:aws:iam::${local.environment_management.account_ids["sprinkler-development"]}:role/terraform-plan-role" : "arn:aws:iam::${local.environment_management.account_ids["sprinkler-development"]}:role/terraform-apply-role"
  }
}
