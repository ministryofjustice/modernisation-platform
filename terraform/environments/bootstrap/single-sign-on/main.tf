...
data "aws_ssoadmin_permission_set" "data_platform_airflow_user" {
  provider = aws.sso-management
...
resource "aws_ssoadmin_account_assignment" "data_platform_airflow_user" {
  ...
  for_each = {
    ...
    if(sso_assignment.level == "data-platform-airflow-user")
  }

  provider = aws.sso-management

  instance_arn       = local.sso_instance_arn
  permission_set_arn = data.aws_ssoadmin_permission_set.data_platform_airflow_user.arn
