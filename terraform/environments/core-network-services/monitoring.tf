...
resource "aws_cloudwatch_metric_alarm" "firewall-traffic-drop-alarm" {
  ...
  statistic                 = "Sum"
  treat_missing_data        = "notBreaching"
  alarm_actions             = [] # Will be populated with a sns topic however currently need to either create a new one or use tgw_monitoring_production or route53_monitoring
