...
#   name        = format("%s-%s", "delivery-stream", random_string.firehose_rnd.result) 
...
#     url                = var.xsiam_firewall_endpoint
#     name               = format("%s-%s", "delivery-stream-endpoint-", random_string.firehose_rnd.result)
#     access_key         = var.xsiam_firewall_secret
#     buffering_size     = 5
...
#   name            =  format("%s-%s", "subscription_filter", random_string.firehose_rnd.result) 
...
#   name              = format("%s-%s", "delivery_errors_log_group", random_string.firehose_rnd.result) 
#   tags = merge(
#     local.tags,
#     {
#       Name = format("%s-%s", "delivery_errors_log_group", random_string.firehose_rnd.result) 
...
#   name           = format("%s-%s", "delivery_errors_log_stream", random_string.firehose_rnd.result)
#   log_group_name = aws_cloudwatch_log_group.delivery_errors_log_group.name
# }
...
#   bucket = format("%s-%s", "firehose_error_logging_bucket", random_string.firehose_rnd.result) 
#   tags   = local.tags
# }
...
#   name  = format("%s-%s", "delivery-stream-role", random_string.firehose_rnd.result)
...
#   name  = format("%s-%s", "delivery-stream-role-policy", random_string.firehose_rnd.result)
#   policy = jsonencode({
#     Version = "2012-10-17"
#     Statement = [
#       {
...
#   name  = format("%s-%s", "error-log-policy", random_string.firehose_rnd.result)
#   policy = jsonencode({
#     Version = "2012-10-17"
#     Statement = [
#       {
...
#   name  = format("%s-%s", "s3-kinesis-xsiam-policy", random_string.firehose_rnd.result)
#   policy = jsonencode({
#     Version = "2012-10-17"
#     Statement = [
#       {
...
#   name_prefix        = format("%s-%s", "xsiam-put-record-role", random_string.firehose_rnd.result) 
#   tags               = local.tags
...
#   name_prefix = format("%s-%s", "xsiam-put-record-policy", random_string.firehose_rnd.result) 
#   tags        = local.tags
#   policy      = <<-EOF
# {
#     "Version": "2012-10-17",
